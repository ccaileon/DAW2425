  ...
  <main class="flex-grow-1">
    ...
    <div class="container mt-5 mb-5">
      ...
      if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $edad = mysqli_real_escape_string($database, $_POST['edad']);
        $email = mysqli_real_escape_string($database, $_POST['email']);
        

        if ( !empty($nombre) && !empty($edad) && !empty($email) ){
          $sql = "INSERT INTO usuarios (nombre, edad, email) 
            VALUES ('$nombre', '$edad', '$email')";


          if (mysqli_query($database, $sql)) {
           echo '
           El usuario se ha añadido correctamente a la base de datos.
           <button class="btn btn-primary mt-4" onclick="location.href=\'addUser.php\'">Volver</button>
           ';
          } else {
            echo 'Error en la consulta: ' . mysqli_error($database);
            echo '<button class="btn btn-primary mt-4" onclick="location.href=\'addUser.php\'">Volver</button>';
          }
        } else {
