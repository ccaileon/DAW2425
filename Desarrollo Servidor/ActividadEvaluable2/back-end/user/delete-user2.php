    ...
    if ($database) {
      ...
      if ($searchUser) {
        ...
        if ($nfilas > 0) {
          ...
          $deleteUser = "DELETE FROM usuario WHERE usuario_id = '$idUser'";
          if (mysqli_query($database, $deleteUser)) {
            echo "Se ha eliminado el usuario correctamente.";
          } else {
            echo "Error al eliminar el usuario: " . mysqli_error($database);
          }
        } else {
